...
def _make_feedforward_classifier(embedding, labels, num_labels, split, num_hidden_layers, extra_features=None,
    ...
    if num_hidden_layers == 0:
        logits = tf.compat.v1.layers.dense(full_embedding, num_labels, activation=None,
                                 kernel_regularizer=regularizer, bias_regularizer=regularizer)

    else:
        layer = tf.compat.v1.layers.dense(full_embedding, 200, activation=tf.nn.elu)
        for _ in range(num_hidden_layers - 1):
            layer = tf.compat.v1.layers.dense(layer, 200, activation=tf.nn.elu,
                                    kernel_regularizer=regularizer, bias_regularizer=regularizer)
        ...
        logits = tf.compat.v1.layers.dense(final_embedding, num_labels, activation=None,
                                 kernel_regularizer=regularizer, bias_regularizer=regularizer)
    ...
    with tf.compat.v1.name_scope("loss"):
        ...
        loss = tf.reduce_sum(input_tensor=censored_per_example_loss)
...
def multiclass_model_fn_builder(bert_config, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        tf.compat.v1.losses.add_loss(masked_lm_loss)
        tf.compat.v1.losses.add_loss(0.1 * label_loss)

        tf.compat.v1.summary.scalar('masked_lm_loss', masked_lm_loss, family='loss')
        tf.compat.v1.summary.scalar('label_loss', label_loss, family='loss')
