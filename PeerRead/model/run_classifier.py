...
def make_regression_loss(include):
    def batch_loss(y_true, y_pred):
        losses = tf.square(y_true - y_pred)
        if include:
            losses = losses[include == 1]
        return tf.reduce_sum(losses)
    return batch_loss


def make_crossent_loss(include=None):
    def batch_loss(y_true, y_pred):
...
def run_bert_classifier(strategy,
    ...
    # TODO: after some thought, it seems impossible to use standard Keras api (loss fn too restrictive), so I need to
    #  figure out the gradient tape thing
